...
To fix this we could memoize `fetcher`'s result with the `useMemo` hook, because `useMemo` will keep the reference to an object.
...
In the Solution, we skip the `fetcher` function in the `useMemo` deps list to prevent an infinite loop.
