...
Again, we lie about the effect dependencies. If we specify `response` as the effect dependency and `response` happens to be a Promise, an infinite loop will occur. The reason is that every time our `useSWR` runs, a brand new Promise object will be created.
