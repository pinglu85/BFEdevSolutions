...
### Approach - O(nlog(n)) - time | O(n) space, where n is total number of time blocks(time intervals) in the input array.
...
### Better Approach - O(n) time | O(n) space, where n is total number of time blocks(time intervals) in the input array.
...
So I also need to initialize a variable that is going to keep track of the previous end time, so that I can know the start time of the current available time block. Initially set it to `0`. When I go through the array of hours, if I get to a value that is not `0` and after adding it to `busyTimeBlockCounter` and `busyTimeBlockCounter` becomes `0`, then the index that the value is at is the new previous end time.

When I get out of the loop and the previous end time is less than `24`, then that means a time block that starts at previous end time and ends at `24` is available for all team members.
