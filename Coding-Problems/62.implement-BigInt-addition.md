...
- Loop over the two numbers from the end to the beginning using the two pointers. The loop terminates when we reach the beginning of both numbers: `p1 >= 0 || p2 >= 0`.
  ...
  - Add `num1[p1]`, `num2[p2]` and the carry up and store the result in a variable `sum`. To handle the case where one of the pointers is out of bounds, we can provide a default value of `0` to both digits.

  - Compute the carry using `Math.floor(sum / 10)`.

  - Get the last digit of the sum using `sum % 10` and push it into the `result` array.
