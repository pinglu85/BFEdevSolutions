...
The `promise` object created by the class `Promise` has a `state` property. The `state` is initially `pending`. So in the constructor of the `MyPromise` class, we need to define a property `state` and set it to `'pending'`.
...
The function is called the **executor**. It is executed immediately and automatically by `new Promise` and is able to `resolve` or `reject` the promise. Hence, we should call it in the `constructor` of the `MyPromise` class with the arguments `resolve` and `reject`.

In addition, if the executor function throws any error, the promise will be rejected. To handle this, we could make use of the `try...catch` statement and reject the promise in the `catch` block.
...
`resolve` and `reject` are both functions which receive one argument. We could define the two methods on the class `MyPromise` with the following signatures:
...
Thus, we could implement our `resolve()` and `reject()` methods like the following:
...
1. **Handling the first argument in our `then()` method**
   ...
   Therefore, the `onFulfilled()` function should be called in the `#resolve()` method, and the `then()` method just registers the `onFulfilled()` function. The `onFulfilled()` function is like a subscriber, subscribing to the promised result, and the `then()` method is kind of like the function `subscribe()` in the Publisher/Subscriber Pattern, which receives subscriber callbacks and stores/registers them in some data structures.
   ...
   Therefore, we need to make the `resolve()` and `reject()` of the promise returned by `then()` available in the `#resolve()` method, so that they can be invoked with the value returned by the `onFulfilled()` function. We could define two properties in `MyPromise` to store both functions:
   ...
   If the `onFulfilled()` function returns a value or `undefined`, we could just call `this.#thenPromiseResolve()` and pass the returned value to it:

   ```js
    #resolve(value) {
      // ...
      queueMicroTask(() => {
        const returnedValue = this.#onFulfilled(this.#result);

        if (!(returnedValue instanceof MyPromise)) {
          this.#thenPromiseResolve(returnedValue);
        } else {
          // do something else
        }
     });
   ...
   If the returned value is an instance of `MyPromise`, we need to wait for the promise to be settled and then resolve/reject the promise returned by the `then()` method. To accomplish this, we could call the `then()` method of the returned value, passing `this.#thenPromiseResolve` and `this.#thenPromiseReject` as the `then` handlers.
   ...
   In addition, we also need to ensure `this.#onFulfilled` is not `undefined` before we call it, since `then()` might not be called:
...
2. **Handling the second argument in our `then()` method**
   ...
   #reject(reason) {
     ...
     queueMicrotask(() => {
       ...
       try {
         const returnedValue = this.#onRejected(this.#result);

         if (!(returnedValue instanceof MyPromise)) {
           this.#thenPromiseResolve(returnedValue);
         } else {
           returnedValue.then(this.#thenPromiseResolve, this.#thenPromiseReject);
         }
       } catch (error) {
         this.#thenPromiseReject(error);
       }
...
In the `Promise`, we can also use the `catch()` method to handle rejected cases, and it also returns a `Promise`.
...
In the `Promise`, the static method `Promise.resolve()` returns a `Promise` object that is resolved with a given value:
...
It is worth pointing out that [Promise.resolve() also handle the case where the value is a thenable](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/resolve#resolving_thenables_and_throwing_errors), but we don't need to cover it here.

In the `MyPromise.resolve()`, we could first check if the value received is an instance of `MyPromise`. If it is, we could just return it. Otherwise we return a new instance of `MyPromise` and resolve the promise with the given value.
...
In the `Promise`, the static method `Promise.reject()` returns a `Promise` object that is rejected with a given reason:
...
2. The `#resolve()` doesn't handle the case when the initial resolved value is a promise. (Credits to [L1ef6Zi](https://bigfrontend.dev/user/L1ef6Zi)
   ...
   To handle this, we could check whether the `value` is an instance of `MyPromise` in the `#resolve()` method. If that is the case, we could call the `value`'s `then()` method and pass in `(value) => { this.#result = value; }` as the parameter:
...
3. The solution doesn't handle multiple `then()` for the same promise. (Credits to [1aN6SC9](https://bigfrontend.dev/user/1aN6SC9))
   ...
   To handle this, we could probably use an array to store the `then` handlers.
