...
Since the `Observer` class is provided for me and I can use `new Observer({next,error,complete})` or `new Observer(function)` to create an observer, I can let `Observer` to take care whether a subscriber is a function or an observer.
